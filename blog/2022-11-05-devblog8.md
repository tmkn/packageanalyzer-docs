...
If not implemented, it will fall back to the TypeScript defined types, but since the library is supposed to be also used without TypeScript and those parameters could come frome the CLI it's better to provide a dedicated `validate` method than to assume that it will always be called with the correct input.
